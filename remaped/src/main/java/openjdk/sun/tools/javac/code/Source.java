...
package openjdk.sun.tools.javac.code;

import static javx.lang.model.SourceVersion.*;
import static openjdk.sun.tools.javac.main.Option.*;
import static openjdk.sun.tools.reflection.android.AndroidSupport.isDalvik;

import java.util.*;
import javx.lang.model.SourceVersion;
import openjdk.sun.tools.javac.jvm.Target;
import openjdk.sun.tools.javac.resources.CompilerProperties.Errors;
import openjdk.sun.tools.javac.resources.CompilerProperties.Fragments;
...
public enum Source {
  ...
  /**
   * 1.5 introduced generics, attributes, foreach, boxing, static import, covariant return, enums,
   * varargs, et al.
   */
  JDK5("5"),
  ...
  /** 13, no language features; text blocks and revised switch expressions in preview */
  JDK13("13"),

  /** 14, switch expressions; pattern matching, records, and revised text blocks in preview */
  JDK14("14"),
   ...
   * which is used to generate error messages of the kind {@code feature XYZ not supported in source
   * N}.
   */
  public enum Feature {
    MODULES(JDK9, Fragments.FeatureModules, DiagKind.PLURAL),
    EFFECTIVELY_FINAL_VARIABLES_IN_TRY_WITH_RESOURCES(
        JDK9, Fragments.FeatureVarInTryWithResources, DiagKind.PLURAL),
    DEPRECATION_ON_IMPORT(MIN, JDK8),
    PRIVATE_SAFE_VARARGS(JDK9),
    DIAMOND_WITH_ANONYMOUS_CLASS_CREATION(
        JDK9, Fragments.FeatureDiamondAndAnonClass, DiagKind.NORMAL),
    ...
    PATTERN_MATCHING_IN_INSTANCEOF(
        JDK16, Fragments.FeaturePatternMatchingInstanceof, DiagKind.NORMAL),
    ...
    UNCONDITIONAL_PATTERN_IN_INSTANCEOF(
        JDK21, Fragments.FeatureUnconditionalPatternsInInstanceof, DiagKind.PLURAL),
