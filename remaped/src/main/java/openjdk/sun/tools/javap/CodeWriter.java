...
import openjdk.sun.tools.classfile.Instruction.TypeKind;
...
public class CodeWriter extends BasicWriter {
  public static CodeWriter instance(Context context) {
    ...
    if (instance == null) instance = new CodeWriter(context);
    return instance;
  }
  ...
  public void writeVerboseHeader(Code_attribute attr, ConstantPool constant_pool) {
    ...
    try {
      ...
      if (!method.access_flags.is(AccessFlags.ACC_STATIC)) ++n; // for 'this'
  ...
  public void writeInstrs(Code_attribute attr) {
    ...
    for (Instruction instr : attr.getInstructions()) {
      try {
        for (InstructionDetailWriter w : detailWriters) w.writeDetails(instr);
        writeInstr(instr);
    ...
    for (InstructionDetailWriter w : detailWriters) w.flush();
  }
  ...
  Instruction.KindVisitor<Void, Integer> instructionPrinter =
      new Instruction.KindVisitor<Void, Integer>() {
        ...
        public Void visitConstantPoolRefAndValue(
            Instruction instr, int index, int value, Integer indent) {
          print("#" + index + ",  " + value);
          tab();
          print("// ");
          printConstant(index);
        ...
        public Void visitTableSwitch(
            Instruction instr, int default_, int low, int high, int[] offsets, Integer indent) {
          int pc = instr.getPC();
          print("{ // " + low + " to " + high);
          indent(indent);
  ...
  public void writeExceptionTable(Code_attribute attr) {
    if (attr.exception_table_length > 0) {
      ...
      for (int i = 0; i < attr.exception_table.length; i++) {
        ...
        print(String.format(" %5d %5d %5d", handler.start_pc, handler.end_pc, handler.handler_pc));
        print("   ");
  ...
  private List<InstructionDetailWriter> getDetailWriters(Code_attribute attr) {
    ...
    if (options.details.contains(InstructionDetailWriter.Kind.SOURCE)) {
      ...
      if (sourceWriter.hasSource()) detailWriters.add(sourceWriter);
      else println("(Source code not available)");
    }
