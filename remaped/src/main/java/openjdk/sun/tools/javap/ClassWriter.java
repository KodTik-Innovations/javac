...
import static openjdk.sun.tools.classfile.ConstantPool.CONSTANT_Module;
import static openjdk.sun.tools.classfile.ConstantPool.CONSTANT_Package;
...
import openjdk.sun.tools.classfile.Field;
import openjdk.sun.tools.classfile.Method;
import openjdk.sun.tools.classfile.Module_attribute;
import openjdk.sun.tools.classfile.Signature;
...
import openjdk.sun.tools.classfile.Type.ArrayType;
import openjdk.sun.tools.classfile.Type.ClassSigType;
import openjdk.sun.tools.classfile.Type.ClassType;
import openjdk.sun.tools.classfile.Type.MethodType;
import openjdk.sun.tools.classfile.Type.SimpleType;
import openjdk.sun.tools.classfile.Type.TypeParamType;
import openjdk.sun.tools.classfile.Type.WildcardType;
...
public class ClassWriter extends BasicWriter {
  ...
  class JavaTypePrinter implements Type.Visitor<StringBuilder, StringBuilder> {
    ...
    public StringBuilder visitClassSigType(ClassSigType type, StringBuilder sb) {
      ...
      if (isInterface) {
        ...
        if (type.superclassType != null && (options.verbose || !type.superclassType.isObject())) {
          sb.append(" extends ");
    ...
    public StringBuilder visitTypeParamType(TypeParamType type, StringBuilder sb) {
      sb.append(type.name);
      String sep = " extends ";
      if (type.classBound != null && (options.verbose || !type.classBound.isObject())) {
        sb.append(sep);
    ...
    private void appendIfNotEmpty(
        StringBuilder sb, String prefix, List<? extends Type> list, String suffix) {
      if (!isEmpty(list)) append(sb, prefix, list, suffix);
    }
  ...
  protected void writeField(Field f) {
    ...
    if (options.showDescriptors) println("descriptor: " + getValue(f.descriptor));
  ...
  protected void writeMethod(Method m) {
    ...
    if (sigAttr == null) {
      ...
      try {
        ...
        if (methodExceptions != null && methodExceptions.isEmpty()) methodExceptions = null;
    ...
    if (classFile.isInterface()
        && (!flags.is(AccessFlags.ACC_ABSTRACT))
        && !name.equals("<clinit>")) {
      if (classFile.major_version > DEFAULT_ALLOWED_MAJOR_VERSION
          || (classFile.major_version == DEFAULT_ALLOWED_MAJOR_VERSION
              && classFile.minor_version >= DEFAULT_ALLOWED_MINOR_VERSION)) {
  ...
  void writeDirectives() {
    Attribute attr = classFile.attributes.get(Attribute.Module);
    if (!(attr instanceof Module_attribute)) return;

    Module_attribute m = (Module_attribute) attr;
    for (Module_attribute.RequiresEntry entry : m.requires) {
      print("requires");
      if ((entry.requires_flags & Module_attribute.ACC_STATIC_PHASE) != 0) print(" static");
      if ((entry.requires_flags & Module_attribute.ACC_TRANSITIVE) != 0) print(" transitive");
      print(" ");
      String mname;
      try {
