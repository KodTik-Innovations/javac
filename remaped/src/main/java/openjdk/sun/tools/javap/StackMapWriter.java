...
import static openjdk.sun.tools.classfile.StackMapTable_attribute.verification_type_info.*;
...
public class StackMapWriter extends InstructionDetailWriter {
  static StackMapWriter instance(Context context) {
    ...
    if (instance == null) instance = new StackMapWriter(context);
    return instance;
  }
  ...
  void setStackMap(StackMapTable_attribute attr) {
    ...
    verification_type_info[] initialLocals =
        new verification_type_info[(isStatic ? 0 : 1) + args.length];
  ...
  void print(verification_type_info entry) {
    ...
    switch (entry.tag) {
      ...
      case ITEM_Object:
        try {
          ConstantPool cp = classWriter.getClassFile().constant_pool;
          ConstantPool.CONSTANT_Class_info class_info =
              cp.getClassInfo(((Object_variable_info) entry).cpool_index);
  ...
  class StackMapBuilder
    ...
    public Integer visit_same_locals_1_stack_item_frame(
        same_locals_1_stack_item_frame frame, Integer pc) {
      int new_pc = pc + frame.getOffsetDelta() + 1;
    ...
    public Integer visit_same_locals_1_stack_item_frame_extended(
        same_locals_1_stack_item_frame_extended frame, Integer pc) {
      int new_pc = pc + frame.getOffsetDelta() + 1;
      StackMap prev = map.get(pc);
      assert (prev != null);
      StackMap m = new StackMap(prev.locals, frame.stack);
    ...
    public Integer visit_append_frame(append_frame frame, Integer pc) {
      ...
      StackMap prev = map.get(pc);
      assert (prev != null);
      verification_type_info[] new_locals =
          new verification_type_info[prev.locals.length + frame.locals.length];
