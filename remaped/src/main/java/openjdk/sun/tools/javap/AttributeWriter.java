...
import static openjdk.sun.tools.classfile.AccessFlags.*;

import java.util.Collection;
import openjdk.sun.tools.classfile.AccessFlags;
import openjdk.sun.tools.classfile.AnnotationDefault_attribute;
import openjdk.sun.tools.classfile.Attribute;
import openjdk.sun.tools.classfile.Attributes;
import openjdk.sun.tools.classfile.BootstrapMethods_attribute;
import openjdk.sun.tools.classfile.CharacterRangeTable_attribute;
import openjdk.sun.tools.classfile.CharacterRangeTable_attribute.Entry;
import openjdk.sun.tools.classfile.Code_attribute;
import openjdk.sun.tools.classfile.CompilationID_attribute;
import openjdk.sun.tools.classfile.ConstantPool;
import openjdk.sun.tools.classfile.ConstantPool.CONSTANT_Class_info;
import openjdk.sun.tools.classfile.ConstantPoolException;
import openjdk.sun.tools.classfile.ConstantValue_attribute;
import openjdk.sun.tools.classfile.DefaultAttribute;
import openjdk.sun.tools.classfile.Deprecated_attribute;
import openjdk.sun.tools.classfile.Descriptor;
import openjdk.sun.tools.classfile.Descriptor.InvalidDescriptor;
import openjdk.sun.tools.classfile.EnclosingMethod_attribute;
import openjdk.sun.tools.classfile.Exceptions_attribute;
import openjdk.sun.tools.classfile.InnerClasses_attribute;
import openjdk.sun.tools.classfile.InnerClasses_attribute.Info;
import openjdk.sun.tools.classfile.LineNumberTable_attribute;
import openjdk.sun.tools.classfile.LocalVariableTable_attribute;
import openjdk.sun.tools.classfile.LocalVariableTypeTable_attribute;
import openjdk.sun.tools.classfile.MethodParameters_attribute;
import openjdk.sun.tools.classfile.ModuleHashes_attribute;
import openjdk.sun.tools.classfile.ModuleMainClass_attribute;
import openjdk.sun.tools.classfile.ModulePackages_attribute;
import openjdk.sun.tools.classfile.ModuleResolution_attribute;
import openjdk.sun.tools.classfile.ModuleTarget_attribute;
import openjdk.sun.tools.classfile.Module_attribute;
import openjdk.sun.tools.classfile.NestHost_attribute;
import openjdk.sun.tools.classfile.NestMembers_attribute;
import openjdk.sun.tools.classfile.PermittedSubclasses_attribute;
import openjdk.sun.tools.classfile.Record_attribute;
import openjdk.sun.tools.classfile.RuntimeInvisibleAnnotations_attribute;
import openjdk.sun.tools.classfile.RuntimeInvisibleParameterAnnotations_attribute;
import openjdk.sun.tools.classfile.RuntimeInvisibleTypeAnnotations_attribute;
import openjdk.sun.tools.classfile.RuntimeParameterAnnotations_attribute;
import openjdk.sun.tools.classfile.RuntimeVisibleAnnotations_attribute;
import openjdk.sun.tools.classfile.RuntimeVisibleParameterAnnotations_attribute;
import openjdk.sun.tools.classfile.RuntimeVisibleTypeAnnotations_attribute;
import openjdk.sun.tools.classfile.Signature_attribute;
import openjdk.sun.tools.classfile.SourceDebugExtension_attribute;
import openjdk.sun.tools.classfile.SourceFile_attribute;
import openjdk.sun.tools.classfile.SourceID_attribute;
import openjdk.sun.tools.classfile.StackMapTable_attribute;
import openjdk.sun.tools.classfile.StackMap_attribute;
import openjdk.sun.tools.classfile.Synthetic_attribute;
import openjdk.sun.tools.classfile.Type;
import openjdk.sun.tools.javac.util.Assert;
import openjdk.sun.tools.javac.util.StringUtils;
...
public class AttributeWriter extends BasicWriter implements Attribute.Visitor<Void, Void> {
  public static AttributeWriter instance(Context context) {
    ...
    if (instance == null) instance = new AttributeWriter(context);
    return instance;
  }
  ...
  public Void visitCharacterRangeTable(CharacterRangeTable_attribute attr, Void ignore) {
    ...
    for (Entry e : attr.character_range_table) {
      print(
          String.format(
              "    %2d, %2d, %6x, %6x, %4x",
              e.start_pc, e.end_pc, e.character_range_start, e.character_range_end, e.flags));
      tab();
      print(
          String.format(
              "// %2d, %2d, %4d:%02d, %4d:%02d",
              e.start_pc,
              e.end_pc,
              (e.character_range_start >> 10),
              (e.character_range_start & 0x3ff),
              (e.character_range_end >> 10),
              (e.character_range_end & 0x3ff)));
      if ((e.flags & CharacterRangeTable_attribute.CRT_STATEMENT) != 0) print(", statement");
      if ((e.flags & CharacterRangeTable_attribute.CRT_BLOCK) != 0) print(", block");
      if ((e.flags & CharacterRangeTable_attribute.CRT_ASSIGNMENT) != 0) print(", assignment");
      if ((e.flags & CharacterRangeTable_attribute.CRT_FLOW_CONTROLLER) != 0)
        print(", flow-controller");
      if ((e.flags & CharacterRangeTable_attribute.CRT_FLOW_TARGET) != 0) print(", flow-target");
      if ((e.flags & CharacterRangeTable_attribute.CRT_INVOKE) != 0) print(", invoke");
      if ((e.flags & CharacterRangeTable_attribute.CRT_CREATE) != 0) print(", create");
      if ((e.flags & CharacterRangeTable_attribute.CRT_BRANCH_TRUE) != 0) print(", branch-true");
      if ((e.flags & CharacterRangeTable_attribute.CRT_BRANCH_FALSE) != 0) print(", branch-false");
      println();
    }
    indent(-1);
    return null;
  ...
  public Void visitEnclosingMethod(EnclosingMethod_attribute attr, Void ignore) {
    ...
    if (attr.method_index != 0) print("." + getMethodName(attr));
    println();
    return null;
  }
  ...
  public Void visitMethodParameters(MethodParameters_attribute attr, Void ignore) {
    ...
    for (MethodParameters_attribute.Entry entry : attr.method_parameter_table) {
      String namestr =
          entry.name_index != 0 ? constantWriter.stringValue(entry.name_index) : "<no name>";
      String flagstr =
  ...
  public Void visitModule(Module_attribute attr, Void ignore) {
    ...
    if ((attr.module_flags & Module_attribute.ACC_OPEN) != 0) print(" ACC_OPEN");
    if ((attr.module_flags & Module_attribute.ACC_MANDATED) != 0) print(" ACC_MANDATED");
    if ((attr.module_flags & Module_attribute.ACC_SYNTHETIC) != 0) print(" ACC_SYNTHETIC");
    println();
  ...
  protected void printRequiresTable(Module_attribute attr) {
    ...
    for (Module_attribute.RequiresEntry e : entries) {
      ...
      if ((e.requires_flags & Module_attribute.ACC_TRANSITIVE) != 0) print(" ACC_TRANSITIVE");
      if ((e.requires_flags & Module_attribute.ACC_STATIC_PHASE) != 0) print(" ACC_STATIC_PHASE");
      if ((e.requires_flags & Module_attribute.ACC_SYNTHETIC) != 0) print(" ACC_SYNTHETIC");
      if ((e.requires_flags & Module_attribute.ACC_MANDATED) != 0) print(" ACC_MANDATED");
      println();
  ...
  protected void printExportOpenEntry(int index, int flags, int[] to_index) {
    ...
    if ((flags & Module_attribute.ACC_MANDATED) != 0) print(" ACC_MANDATED");
    if ((flags & Module_attribute.ACC_SYNTHETIC) != 0) print(" ACC_SYNTHETIC");
  ...
  public Void visitModuleResolution(ModuleResolution_attribute attr, Void ignore) {
    ...
    if ((flags & ModuleResolution_attribute.WARN_DEPRECATED) != 0) print(" WARN_DEPRECATED");
    if ((flags & ModuleResolution_attribute.WARN_DEPRECATED_FOR_REMOVAL) != 0)
      print(" WARN_DEPRECATED_FOR_REMOVAL");
    if ((flags & ModuleResolution_attribute.WARN_INCUBATING) != 0) print(" WARN_INCUBATING");
    println();
    indent(-1);
    return null;
  }
  ...
  public Void visitRecord(Record_attribute attr, Void p) {
    ...
    for (Record_attribute.ComponentInfo componentInfo : attr.component_info_arr) {
      Signature_attribute sigAttr =
          (Signature_attribute) componentInfo.attributes.get(Attribute.Signature);

      if (sigAttr == null) print(getJavaFieldType(componentInfo.descriptor));
      else {
        try {
  ...
  public Void visitRuntimeInvisibleAnnotations(
      RuntimeInvisibleAnnotations_attribute attr, Void ignore) {
  ...
  @Override
  public Void visitRuntimeVisibleTypeAnnotations(
      RuntimeVisibleTypeAnnotations_attribute attr, Void ignore) {
  ...
  @Override
  public Void visitRuntimeInvisibleTypeAnnotations(
      RuntimeInvisibleTypeAnnotations_attribute attr, Void ignore) {
  ...
  private void visitParameterAnnotations(
      String message, RuntimeParameterAnnotations_attribute attr) {
    println(message);
    indent(+1);
  ...
  public Void visitRuntimeVisibleParameterAnnotations(
      RuntimeVisibleParameterAnnotations_attribute attr, Void ignore) {
    visitParameterAnnotations(
        "RuntimeVisibleParameterAnnotations:", (RuntimeParameterAnnotations_attribute) attr);
    return null;
  }

  @Override
  public Void visitRuntimeInvisibleParameterAnnotations(
      RuntimeInvisibleParameterAnnotations_attribute attr, Void ignore) {
    visitParameterAnnotations(
        "RuntimeInvisibleParameterAnnotations:", (RuntimeParameterAnnotations_attribute) attr);
    return null;
  }
  ...
  implements StackMapTable_attribute.stack_map_frame.Visitor<Void, Void> {
    ...
    @Override
    public Void visit_same_locals_1_stack_item_frame(
        StackMapTable_attribute.same_locals_1_stack_item_frame frame, Void p) {
    ...
    public Void visit_same_locals_1_stack_item_frame_extended(
        StackMapTable_attribute.same_locals_1_stack_item_frame_extended frame, Void p) {
    ...
    @Override
    public Void visit_same_frame_extended(
        StackMapTable_attribute.same_frame_extended frame, Void p) {
