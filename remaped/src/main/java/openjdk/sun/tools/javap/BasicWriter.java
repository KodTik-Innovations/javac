...
public class BasicWriter {
  ...
  protected BasicWriter(Context context) {
    ...
    if (messages == null) throw new AssertionError();
  }
  ...
  protected String space(int w) {
    if (w < spaces.length && spaces[w] != null) return spaces[w];

    StringBuilder sb = new StringBuilder();
    for (int i = 0; i < w; i++) sb.append(" ");

    String s = sb.toString();
    if (w < spaces.length) spaces[w] = s;
  ...
  private static class LineWriter {
    static LineWriter instance(Context context) {
      ...
      if (instance == null) instance = new LineWriter(context);
      return instance;
    }
      ...
      for (int i = 0; i < s.length(); i++) {
        ...
        switch (c) {
          ...
          default:
            ...
            if (pendingSpaces > 0) {
              for (int sp = 0; sp < pendingSpaces; sp++) buffer.append(' ');
              pendingSpaces = 0;
            }
            buffer.append(c);
        }
