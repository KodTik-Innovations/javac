...
/**
 * "Main" class for javap, normally accessed from the command line via Main, or from JSR199 via
 * DisassemblerTool.
 *
...
public class JavapTask implements DisassemblerTool.DisassemblerTask, Messages {
    ...
    new Option(false, "-XDdetails:") {
      ...
      void process(JavapTask task, String opt, String arg) throws BadArgs {
        ...
        for (String v : opt.substring(sep + 1).split("[,: ]+")) {
          if (!handleArg(task, v)) throw task.new BadArgs("err.invalid.arg.for.option", v);
        }
      }
      ...
      boolean handleArg(JavapTask task, String arg) {
        ...
        for (InstructionDetailWriter.Kind k : InstructionDetailWriter.Kind.values()) {
          if (arg.equalsIgnoreCase(k.option)) {
            if (on) task.options.details.add(k);
            else task.options.details.remove(k);
            return true;
          }
        }
        return false;
  ...
  public JavapTask(
    ...
    try {
      if (options != null) handleOptions(options, false);
    } catch (BadArgs e) {
  ...
  private static PrintWriter getPrintWriterForWriter(Writer w) {
    if (w == null) return getPrintWriterForStream(null);
    else if (w instanceof PrintWriter) return (PrintWriter) w;
    else return new PrintWriter(w, true);
  }
    ...
    try {
      try {
        ...
        if (classes == null || classes.size() == 0) {
          if (options.help || options.version || options.fullVersion) return EXIT_OK;
          else return EXIT_CMDERR;
        }
      ...
      Object[] e_args;
      if (e.getCause() == null) e_args = e.args;
      else {
  ...
  private void handleOptions(Iterable<String> args, boolean allowClasses) throws BadArgs {
    if (log == null) {
      ...
      if (diagnosticListener == null) diagnosticListener = getDiagnosticListenerForWriter(log);
    }

    if (fileManager == null) fileManager = getDefaultFileManager(diagnosticListener, log);
    ...
    while (iter.hasNext()) {
      ...
      if (arg.startsWith("-")) handleOption(arg, iter);
      else if (allowClasses) {
        if (classes == null) classes = new ArrayList<>();
        classes.add(arg);
        while (iter.hasNext()) classes.add(iter.next());
      } else throw new BadArgs("err.unknown.option", arg).showUsage(true);
    }
    ...
    if (!options.showDisassembled && !options.verbose && options.showLineAndLocalVariableTables) {
      reportWarning(
          ...
          "-l without -c, line number and local variable tables will not be printed");
    }
    ...
    if (noArgs || options.help) showHelp();

    if (options.version || options.fullVersion) showVersion(options.fullVersion);
  }
  ...
  private void handleOption(String name, Iterator<String> rest) throws BadArgs {
    for (Option o : recognizedOptions) {
      if (o.matches(name)) {
        if (o.hasArg) {
          if (rest.hasNext()) o.process(this, name, rest.next());
          else throw new BadArgs("err.missing.arg", name).showUsage(true);
        } else o.process(this, name, null);

        if (o.ignoreRest()) {
          while (rest.hasNext()) rest.next();
        }
        return;
      }
    }
    ...
    try {
      if (fileManager.handleOption(name, rest)) return;
      ...
      if (a instanceof InnerClasses_attribute) {
        ...
        try {
          ...
          for (int i = 0; i < inners.classes.length; i++) {
            ...
            ConstantPool.CONSTANT_Class_info outerClassInfo =
                cf.constant_pool.getClassInfo(outerIndex);
            ...
            if (outerClassName.equals(cf.getName())) {
              ...
              ConstantPool.CONSTANT_Class_info innerClassInfo =
                  cf.constant_pool.getClassInfo(innerIndex);
  ...
  private JavaFileManager getDefaultFileManager(
      final DiagnosticListener<? super JavaFileObject> dl, PrintWriter log) {
    if (defaultFileManager == null) defaultFileManager = JavapFileManager.create(dl, log);
    ...
    try {
      ...
      if (moduleLocation != null) {
        ...
        if (className.indexOf('.') > 0 || className.indexOf('/') > 0) {
          // search for classes with a named package in the JDK modules specifed by --system option
          // first
          try {
            for (Set<Location> locations :
                fileManager.listLocationsForModules(StandardLocation.SYSTEM_MODULES)) {
              for (Location systemModule : locations) {
                fo =
                    fileManager.getJavaFileForInput(
                        systemModule, className, JavaFileObject.Kind.CLASS);
                if (fo != null) return fo;
              }
            }
      ...
      for (Set<Location> set : fileManager.listLocationsForModules(segment)) {
        ...
        for (Location l : set) {
          ...
          if (name.equals(moduleName)) {
            ...
            else throw new IOException("multiple definitions found for " + moduleName);
          }
        }
  ...
  private void showHelp() {
    ...
    for (String o : fmOptions) {
      if (fileManager.isSupportedOption(o) == -1) continue;
