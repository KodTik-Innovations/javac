...
import openjdk.sun.tools.classfile.Code_attribute.Exception_data;
...
public class TryBlockWriter extends InstructionDetailWriter {
  ...
  static TryBlockWriter instance(Context context) {
    ...
    if (instance == null) instance = new TryBlockWriter(context);
    return instance;
  }
    ...
    if (entries != null) {
      for (ListIterator<Exception_data> iter =
        ...
        if (kind.match(entry, pc)) {
          ...
          print("] ");
          if (entry.catch_type == 0) print("finally");
          else {
