...
import openjdk.sun.tools.classfile.RuntimeTypeAnnotations_attribute;
import openjdk.sun.tools.classfile.RuntimeVisibleTypeAnnotations_attribute;
import openjdk.sun.tools.classfile.TypeAnnotation;
import openjdk.sun.tools.classfile.TypeAnnotation.Position;
...
public class TypeAnnotationWriter extends InstructionDetailWriter {
  ...
  static TypeAnnotationWriter instance(Context context) {
    ...
    if (instance == null) instance = new TypeAnnotationWriter(context);
    return instance;
  }
  ...
  public void reset(Code_attribute attr) {
    ...
    check(
        ...
        (RuntimeVisibleTypeAnnotations_attribute)
            m.attributes.get(Attribute.RuntimeVisibleTypeAnnotations));
    check(
        NoteKind.INVISIBLE,
        (RuntimeInvisibleTypeAnnotations_attribute)
            m.attributes.get(Attribute.RuntimeInvisibleTypeAnnotations));
  }
  ...
  private void check(NoteKind kind, RuntimeTypeAnnotations_attribute attr) {
    ...
    for (TypeAnnotation anno : attr.annotations) {
      ...
      Note note = null;
      if (p.offset != -1) addNote(p.offset, note = new Note(kind, anno));
      if (p.lvarOffset != null) {
        for (int i = 0; i < p.lvarOffset.length; i++) {
          if (note == null) note = new Note(kind, anno);
  ...
  private void addNote(int pc, Note note) {
    ...
    if (list == null) pcMap.put(pc, list = new ArrayList<>());
    list.add(note);
  }
