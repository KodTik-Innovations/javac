...
/** Represents a method's signature, that is, its parameter types and its return type. */
public class MethodSig {
  ...
  private static IllegalArgumentException ex(String desc, int pos) {
    return new IllegalArgumentException(
        String.format("illegal descriptor \"%s\" at position %d", desc, pos));
  }

  private static MethodSig parse(String desc, int start, int end) throws IllegalArgumentException {
    int p = start;
    int dims = 0;
    ...
    while (p < end) {
      ...
      switch (ch = desc.charAt(p)) {
        ...
        case 'L':
          ...
          if (sep == -1 || sep >= end) throw ex(desc, p);
